<script lang="ts" generics="TData, TValue">
	...
	let filterValue = $derived(
		(rawFilterValue as ColumnFilterState | undefined) ?? {
	...
	<PopoverTrigger>
		{#snippet child({ props })}
			<Button
				...
				variant="ghost"
				size="icon-sm"
				class="ml-1 h-6 w-6 p-0 {isFilterActive ? 'text-primary' : 'text-muted-foreground'}"
			>
		...
		<div class="space-y-4">
			...
			{#if filterDef.type === 'enum' && filterDef.options}
				<div class="space-y-2">
					...
					<div class="max-h-[150px] space-y-2 overflow-y-auto rounded-md border p-2">
						{#each filterDef.options as option}
							{@const checked =
								Array.isArray(filterValue.value) && filterValue.value.includes(option.value)}
